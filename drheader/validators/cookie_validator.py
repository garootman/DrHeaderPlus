...
class CookieValidator(base.ValidatorBase):
    ...
    def value_gte(self, config: CaseInsensitiveDict[str, Any], header: str, **kwargs: Any) -> Never:
        ...
        Raises:
            UnsupportedValidationError: If the method is called.
        """
        raise base.UnsupportedValidationError("'Value-Gte' validations are not supported for cookies")
    ...
    def must_avoid(self, config: CaseInsensitiveDict[str, Any], header: str, **kwargs: Any) -> ReportItem | None:
        ...
        if anomalies:
            ...
            return ReportItem(
                severity, error_type, header, cookie=cookie, value=cookie_value, avoid=disallowed, anomalies=anomalies
            )  # noqa:E501
