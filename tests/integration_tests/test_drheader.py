...
class TestDrHeader(unittest.TestCase):
    ...
    def test_header__should_handle_case_insensitive_header_names(self):
        rules = {"Content-Security-Policy": {"Required": True}}
        ...
        report = utils.process_test(headers=headers, rules=rules)
    ...
    def test_header__should_handle_case_insensitive_header_values(self):
        rules = {"Content-Security-Policy": {"Required": True, "Must-Contain": ["default-src"]}}
        headers = utils.add_or_modify_header("Content-Security-Policy", "default-src 'none'")
    ...
    def test_header__should_not_validate_an_optional_header_that_is_not_present(self):
        rules = {"Cache-Control": {"Required": "Optional", "Value": ["no-store"]}}
    ...
    def test_header__exists_validation_ko(self):
        rules = {"Cache-Control": {"Required": True}}
        headers = utils.delete_headers("Cache-Control")
    ...
    def test_header__not_exists_validation_ko(self):
        rules = {"Cache-Control": {"Required": False}}
    ...
    def test_header__value_validation_ko(self):
        rules = {"Cache-Control": {"Required": True, "Value": ["no-store"]}}
        headers = utils.add_or_modify_header("Cache-Control", "no-cache")
    ...
    def test_header__value_any_of_validation_ko(self):
        rules = {"Cache-Control": {"Required": True, "Value-Any-Of": ["private", "no-cache", "no-transform"]}}
    ...
    def test_header__value_one_of_validation_ko(self):
        rules = {"Cache-Control": {"Required": True, "Value-One-Of": ["no-cache", "no-store"]}}
    ...
    def test_header__must_avoid_validation_ko(self):
        rules = {"Cache-Control": {"Required": True, "Must-Avoid": ["private", "public"]}}
    ...
    def test_header__must_contain_validation_ko(self):
        rules = {"Cache-Control": {"Required": True, "Must-Contain": ["must-revalidate"]}}
    ...
    def test_header__must_contain_one_validation_ko(self):
        rules = {"Cache-Control": {"Required": True, "Must-Contain-One": ["must-revalidate", "no-cache"]}}
        headers = utils.add_or_modify_header("Cache-Control", "private")
    ...
    def test_directive__value_validation_ko(self):
        rules = {
            "Content-Security-Policy": {
                "Required": True,
                "Directives": {"style-src": {"Required": True, "Value": ["self"]}},
            },
        }
    ...
    def test_directive__value_any_of_validation_ko(self):
        rules = {
            "Content-Security-Policy": {
                "Required": True,
                "Directives": {
                    "style-src": {"Required": True, "Value-Any-Of": ["https://example1.com", "https://example2.com"]}
                },
            },
        }
        headers = utils.add_or_modify_header("Content-Security-Policy", "style-src https://example.com")
    ...
    def test_directive__value_one_of_validation_ko(self):
        rules = {
            "Content-Security-Policy": {
                "Required": True,
                "Directives": {"style-src": {"Required": True, "Value-One-Of": ["none", "self"]}},
            },
        }
        headers = utils.add_or_modify_header("Content-Security-Policy", "style-src https://example.com")
    ...
    def test_directive__must_avoid_validation_ko(self):
        rules = {
            "Content-Security-Policy": {
                "Required": True,
                "Directives": {"style-src": {"Required": True, "Must-Avoid": ["unsafe-inline"]}},
            },
        }
    ...
    def test_directive__must_contain_validation_ko(self):
        rules = {
            "Content-Security-Policy": {
                "Required": True,
                "Directives": {"style-src": {"Required": True, "Must-Contain": ["https://example.com"]}},
            },
        }
        ...
        rules = {
            "Content-Security-Policy": {
                ...
                "Directives": {
                    "style-src": {
                        "Required": True,
                        "Must-Contain-One": ["https://example1.com", "https://example2.com"],
                    }
                },
            },
        }
    ...
    def test_cookie__exists_validation_ko(self):
        rules = {"Set-Cookie": {"Required": True, "Cookies": {"session": {"Required": True}}}}
    ...
    def test_cookie__not_exists_validation_ko(self):
        rules = {"Set-Cookie": {"Required": True, "Cookies": {"session": {"Required": False}}}}
    ...
    def test_cookie__must_avoid_validation_ko(self):
        rules = {"Set-Cookie": {"Required": True, "Cookies": {"session": {"Required": True, "Must-Avoid": ["Path"]}}}}
    ...
    def test_cookie__must_contain_validation_ko(self):
        rules = {
            "Set-Cookie": {
                "Required": True,
                "Cookies": {"session": {"Required": True, "Must-Contain": ["HttpOnly", "SameSite=Strict", "Secure"]}},
            },
        }
    ...
    def test_cookie__must_contain_one_validation_ko(self):
        rules = {
            "Set-Cookie": {
                "Required": True,
                "Cookies": {"session": {"Required": True, "Must-Contain-One": ["Expires", "Max-Age"]}},
            },
        }
        headers = utils.add_or_modify_header("Set-Cookie", ["session=657488329; HttpOnly; Secure"])
...
class TestCorsOriginProbe(unittest.TestCase):
    @responses.activate
    def test_cors__reflected_origin_should_fail(self):
        url = "https://example.com"
        # Initial HEAD request (for Drheader constructor)
        responses.add(
            responses.HEAD,
            url,
            status=200,
        ...
        # CORS probe GET request — reflects origin
        responses.add(
            responses.GET,
            url,
            status=200,
        ...
        drheader = core.Drheader(url=url)
        report = drheader.analyze(rules={"Access-Control-Allow-Origin": {"Required": "Optional", "Severity": "high"}})
        expected = Finding(
            rule="Access-Control-Allow-Origin",
            message="Access-Control-Allow-Origin reflects arbitrary Origin (potential CORS misconfiguration)",
            severity="medium",
    ...
    @responses.activate
    def test_cors__reflected_origin_with_credentials_should_be_high_severity(self):
        url = "https://example.com"
        responses.add(
            responses.HEAD,
            url,
        ...
        drheader = core.Drheader(url=url)
        report = drheader.analyze(rules={"Access-Control-Allow-Origin": {"Required": "Optional", "Severity": "high"}})
        cors_findings = [f for f in report if f.rule == "Access-Control-Allow-Origin" and "reflects" in f.message]
        self.assertEqual(len(cors_findings), 1)
        self.assertEqual(cors_findings[0].severity, "high")

    @responses.activate
    def test_cors__non_reflected_origin_should_pass(self):
        url = "https://example.com"
        responses.add(
            responses.HEAD,
            url,
        ...
        drheader = core.Drheader(url=url)
        report = drheader.analyze(rules={"Access-Control-Allow-Origin": {"Required": "Optional", "Severity": "high"}})
        cors_findings = [f for f in report if "reflects" in f.message]
        self.assertEqual(len(cors_findings), 0)

    def test_cors__no_url_should_skip_probe(self):
        """CORS probe should not run in compare mode (no URL)."""
        headers = {"Access-Control-Allow-Origin": "*"}
        drheader = core.Drheader(headers=headers)
        report = drheader.analyze(
            rules={"Access-Control-Allow-Origin": {"Required": "Optional", "Severity": "high", "Must-Avoid": ["*"]}}
        )
        cors_reflection_findings = [f for f in report if "reflects" in f.message]
        self.assertEqual(len(cors_reflection_findings), 0)
